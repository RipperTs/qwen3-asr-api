...
OFFLINE_WORKER_COUNT = 1        # 离线任务 worker 数；仅支持 batch_transcribe 的 ASR 后端可 >1
OFFLINE_ASR_BATCH_SIZE = 32     # 全局离线 ASR scheduler 的 batch 上限，不改变单 chunk 内容
OFFLINE_BATCH_WAIT_MS = 100     # 全局离线 ASR scheduler 为跨任务合批等待的最大窗口
...
VLLM_MAX_UTTERANCE_SEC = 20         # 前端输出分段（秒）：长句兜底切卡片，便于回看定位
VLLM_MAX_STATE_SEC = 300            # SDK state 生命周期上限（秒）：重建 state，约束 audio_accum/prefix 膨胀
