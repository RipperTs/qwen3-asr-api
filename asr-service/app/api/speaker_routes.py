...
409 认领冲突（结构化冲突对象）｜ 503 speaker_db_disabled（模块未启用）/
model_tag_mismatch（仅禁登记/识别/认领，管理端点保留——被遗忘权不受失配影响）｜
500 其余 SpeakerStoreError。
"""
import asyncio
import logging
import os
...
async def _run(fn, *args):
    ...
    except SpeakerClaimConflictError as e:
        raise HTTPException(
            status_code=409,
            detail={
                "code": e.code,
...
async def claim_speaker(
    ...
    """认领自动登记说话人并以管理员样本原子替换模板。"""
    service = _require_writable()
    if consent is not True:
        raise HTTPException(
            status_code=400,
            detail="认领必须携带 consent=true（确认已获得数据主体同意）",
        )
    paths = []
    try:
        for file in files:
        ...
        return SpeakerClaimResponse(**result)
    finally:
        _cleanup(paths)
...
def build_speakers_router() -> APIRouter:
    ...
    r.add_api_route("/speakers/{speaker_id}/claim", claim_speaker, methods=["POST"],
                    response_model=SpeakerClaimResponse, dependencies=dep)
