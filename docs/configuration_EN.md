...
| Speaker diarization/ID | supported | **offline + realtime final supported** (CAM++, requires `--enable-speaker`; energy-based segmentation); partials carry no speaker |
...
- **Speaker diarization/ID**: with `--enable-speaker` (plus `--enable-speaker-db` for the voiceprint DB), offline `segments[].speaker` / `speaker_name` / `speakers` match standard; realtime emits `speaker` on sentence-level `final` events and `speaker_name` when a voiceprint matches, while `partial` carries no speaker and historical events are never revised. The engine is CAM++ (CPU, torch, not funasr); both offline windowing and realtime endpointing use energy detection instead of FSMN-VAD (coarser boundaries). When disabled, `diarize`/`identify_speakers` are recorded in `warnings`. Requires extra deps `scipy`/`scikit-learn`/`modelscope` (or a pre-mounted CAM++ model dir); see [requirements-vllm.txt](../asr-service/requirements-vllm.txt).
> Use `standard` mode when FSMN-precise segmentation or independently controlled CT-Transformer punctuation is required.
