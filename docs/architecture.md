...
- **任务队列**：默认单工作线程串行处理 + 线程池真超时；支持 `batch_transcribe` 的 ASR 后端可通过离线 worker 配置让多个任务同时推进。ASR 阶段由全局动态合批调度器统一调用单模型 `batch_transcribe`，只合并多个任务的 chunk，不改变单个 chunk 的音频内容、语言参数和结果解析；不支持批处理的后端启动时退回单 worker；可选 write-through 持久化（[tasks.db](configuration.md#离线任务持久化tasksdb)），持久化故障不影响任务执行。
