...
> **Compatibility APIs (OpenAI/DashScope) & speakers**: vLLM mode also supports `--enable-openai-api`/`--enable-dashscope-api` (offline + realtime WS) and `--enable-speaker`/`--enable-speaker-db` (speaker diarization/identification in offline output and realtime finals, CAM++; energy-detection boundaries are weaker than standard's FSMN). Key difference vs standard — vLLM streaming is always on, so **realtime compat auto-mounts with the compat switches and needs no `--enable-stream`**, and realtime supports per-token increments (DashScope intermediate results are forwarded cleanly and finals may carry `speaker_id`; OpenAI delta is best-effort and has no speaker field). Local launch uses the isolated env (not the default venv): `venv-vllm/bin/python -m app.main --serve-mode vllm …` (Docker is wrapped by the compose above; vLLM requires CUDA and exits on non-GPU devices). See the [compatibility API docs](api/compat_EN.md) for capabilities and protocol.
